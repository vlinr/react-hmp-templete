...
function useHeartTimer(
    ...
    // 开启心跳
    const startHeart = (callback?: Function) => {
        ...
        timeRef.current.timer = setInterval(() => {
            ...
            if (timeRef.current.timeNum >= timeRef?.current?.jumpHeartDeeps) {
                // 步长超过了心跳的值，进行回调，重置计时
                callback?.();
                timeRef.current.timeNum =
                    timeRef.current.timeNum - timeRef?.current?.jumpHeartDeeps; // 超过的内容，作为下一次的时间
            }
        }, deeps);
    };
    ...
    // 暂停心跳
    const pauseHeart = () => {
        if (timeRef.current) timeRef.current.pause = true;
    };

    // 恢复心跳
    const resumeHeart = () => {
        if (timeRef.current) timeRef.current.pause = false;
    };
