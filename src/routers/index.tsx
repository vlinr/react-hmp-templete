        ...
        <Switch>
            ...
            }
            {
                FLATTEN_ROUTER?.map((item, index) => {
                    return item.component && ((item?.authority || [])?.indexOf(USER_AUTHORITY) > -1 || (item?.authority || [])?.length === 0) && <AuthRouter exact key={index} name={item?.name} path={item.path} isNotLogin={item.isNotLogin} component={item.component} layout={item.layout} ></AuthRouter>
                })
            }
            <Redirect to="/404" />
        </Switch>
